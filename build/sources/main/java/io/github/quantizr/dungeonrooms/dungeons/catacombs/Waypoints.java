...
import io.github.quantizr.dungeonrooms.CustomWaypoints.CustomPoint;
...
public class Waypoints {
    ...
    public static boolean customWaypointsOn = true;
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        if(cP != null && !cP.isEmpty() && customWaypointsOn){
            for(CustomPoint p : cP){
                ...
                Color color = new Color(190, 255, 252);
                double viewerX = viewer.lastTickPosX + (viewer.posX - viewer.lastTickPosX) * event.partialTicks;
                double viewerY = viewer.lastTickPosY + (viewer.posY - viewer.lastTickPosY) * event.partialTicks;
                double viewerZ = viewer.lastTickPosZ + (viewer.posZ - viewer.lastTickPosZ) * event.partialTicks;
                ...
                GlStateManager.disableTexture2D();
                if (showBeacon && distSq > 5*5) WaypointUtils.renderBeaconBeam(x, y + 1, z, color.getRGB(), 0.25f, event.partialTicks);
                if (showWaypointText) WaypointUtils.renderWaypointText(p.getName(), pos.up(2), event.partialTicks);
                GlStateManager.disableLighting();
                GlStateManager.enableTexture2D();
                GlStateManager.enableDepth();
                GlStateManager.enableCull();
